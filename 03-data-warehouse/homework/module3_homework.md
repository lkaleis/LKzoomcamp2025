...
```sql
CREATE OR REPLACE EXTERNAL TABLE `neon-runway-447221-q8.dezoomcamp.yellow_trips2024`
OPTIONS (
  format = 'PARQUET',
  uris = ['gs://lindakaleis-kestra/yellow_tripdata_2024-*.parquet']
);
...
from dezoomcamp.yellow_trips2024;
-- query from regular/materialized table
select count(distinct PULocationID)
...
CREATE OR REPLACE TABLE `neon-runway-447221-q8.dezoomcamp.yellow_trips2024_partitioned`
PARTITION BY DATE(tpep_dropoff_datetime)
CLUSTER BY VendorID AS (
  SELECT * FROM `neon-runway-447221-q8.dezoomcamp.yellow_trips2024_reg`
);
...
select count(distinct VendorID)
from dezoomcamp.yellow_trips2024_partitioned
where tpep_dropoff_datetime>= '2024-03-01' and tpep_dropoff_datetime <='2024-03-15'
```
